...
class ClassifyFst(GraphFst):
    """
    Final class that composes all other classification grammars for Persian ITN.
    This class can process an entire sentence and convert spoken form to written form.
    ...
    def __init__(
        ...
        if cache_dir is not None and cache_dir != "None":
            ...
            far_file = os.path.join(cache_dir, f"fa_itn_{input_case}.far")
        if not overwrite_cache and far_file and os.path.exists(far_file):
            self.fst = pynini.Far(far_file, mode="r")["tokenize_and_classify"]
            logger.info(f"ClassifyFst.fst was restored from {far_file}.")
        else:
            logger.info(f"Creating ClassifyFst grammars for Persian ITN. This might take some time...")

            # Initialize TN classifier to get the grammars to invert
            ...
            time = TimeFst(tn_cardinal=tn_classify.cardinal)
            time_graph = time.fst

            date = DateFst(tn_cardinal=tn_classify.cardinal)
            date_graph = date.fst
